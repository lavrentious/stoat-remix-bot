...
class Remix {
  ...
  async generateVisitorData() {
    try {
      ...
      console.log("[Innertube init] VisitorData fetched: ", visitorData);
      return visitorData;
    } catch (e) {
      console.log("[Innertube init] VisitorData fetch failed. Retrying in 2 seconds.", e?.message);
      await new Promise(res => setTimeout(res, 2000));
      return this.generateVisitorData();
    }
  }
  ...
  async initInnertube() {
    ...
    Platform.shim.eval = async (data, _env) => {
      // data.output already contains the full script + `return process(n, sp, s)` appended by youtubei.js
